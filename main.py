...
def plotsounds(ax, data, abszisse, param_dict=None, filename='N/A', samplerate=48000, log=True,
    ...
    :param samplerate: 96000 sind Standard. Wird fÃ¼r die Zeitachse genommen
...
if __name__ == "__main__":
    ...
    ################################################
    # Zuweisung und ÃœberprÃ¼fung des Ordners anhand verschiedener Test-Variablen
    ################################################
    ...
    csvlength = 300  # Achtung es werden die Zeilen 2x gezÃ¤hlt -> 50 dateien = 100 zeilen
    wavfiles = np.random.choice(wavfiles,5)
    ################################################
    # SchÃ¤tzung unbekannter Parameter Ã¼ber die t-verteilte Grundgesamtheit
    ################################################
    ...
    #############################################
    # entweder Ã¼ber Quantile oder harter Schwellenwert..
    qu = pwr['GesamtEnergie'].quantile(0.1)
    qo = pwr['GesamtEnergie'].quantile(0.999)
    v=pwr.shape[0]
    minimalschwellle = 0.3  # eigentlich lieber mit der fft fÃ¼r frequenzen kleiner 300 Hz
    maximalschwelle = 100
    ...
    ################################################
    win, step = 0.005, 0.005  # Laufendes Fenster, keine Ãœberlappung
    if not do_test_mode:
        for audiofile, row in wfdf.iterrows():
            ...
            iteration_over_file = 0
            ################################################
            # Laufe 2x Ã¼ber das Signal. Es stecken meistens 2 Ticks pro File
            ################################################
            ...
            while iteration_over_file < 2:
                ...
                ################################################
                # Versuche das geschnittene Signal in die csv zu drÃ¼cken.. Wenn es nicht, da nicht gleich lang so passt
                # das Programm das geschnittene Signal an und fÃ¼llt es mit einem konstanten Wert..
                #
                ################################################
                try:
                    ...
                    print('Diese Datei muss nÃ¤her untersucht werden:\t' + audiofile)
                    # TatsÃ¤chlich habe ich die Dateien bereits verworfen gehabt.. aber dann doch wieder eingebaut..
                    # try:
                    ...
                    #    pass
                ################################################
                # Export der csv datei wenn lÃ¤nger als x
                ################################################
                # TODO: wenn Daten fÃ¼r vollstÃ¤ndige CSV nicht mehr langen, dann sollte dies ebenfalls behandelt werden
    ...
    ### TODO: Das hier muss an die richtige Stelle geschoben werden :-) Ist nur zum testen um die AusfÃ¼hrung an einer geeigneten Stelle zu unterbrechen
    if do_test_mode:
        plt.close('all')
        exit(1)
'''
